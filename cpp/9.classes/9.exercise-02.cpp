...
void Name_pairs::read_names()
{
  ...
  while (cin >> name && name != stop) {
    if (is_in(name))
      cout << "Duplicated name " << name << "! Try again...\n";
    else
...
void Name_pairs::read_ages()
{
  ...
  for (string name : names) {
    ...
    double age {0};
    while (!(cin >> age)) {
      cout << "Sorry, I don't understand you! How old is " << name << "? ";
      cin.clear();
...
void Name_pairs::sort()
{
  if (names.size() != ages.size())
    error("names and ages sizes differ");
  ...
  for (size_t i = 0; i < limit; ++i)
    sorted_ages[i] = ages[name_idx(sorted_names[i])];
